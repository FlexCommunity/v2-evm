...
contract EcoPythCalldataBuilder3_ForkTest is DynamicForkBaseTest {
  ...
  function _setupCIX1PriceAdapter() internal {
    ...
    ecoPyth2.insertAssetId("EUR");
    ecoPyth2.insertAssetId("JPY");
    ecoPyth2.insertAssetId("GBP");
    ecoPyth2.insertAssetId("CAD");
    ecoPyth2.insertAssetId("CNH");
    ecoPyth2.insertAssetId("CHF");
    vm.stopPrank();
    ...
    */
    ...
    // uint256[] memory _weightsE8 = new uint256[](3);
    // _weightsE8[0] = 0.55e8;
    // _weightsE8[1] = 0.15e8;
    // _weightsE8[2] = 0.3e8;
    ...
    vm.startPrank(multiSig);
    ecoPyth2.insertAssetId("CIX1");
    vm.stopPrank();
  }

  function testCorrectness_CalcPriceLens_getPrice() external onlyFork {
  ...
  function testCorrectness_EcoPythCalldataBuilder3_build() external onlyFork {
    // Prepare build data
    ...
    _overwritePriceByAssetId("JPY", _data, 1.4939e8, 15_000);
    ...
    vm.startPrank(address(botHandler));
    ...
    // Assert CIX
    {
      PythStructs.Price memory _p = ecoPyth2.getPriceUnsafe("CIX1");
      assertApproxEqRel(_p.price, 45.52260850e8, 0.000001e18, "CIX1 price should be 100 USD"); // 0.000001% error
    }

    // Assert wstETH
    // {
    //   PythStructs.Price memory _p = ecoPyth2.getPriceUnsafe("wstETH");
    ...
    // Randomly assert the common assets, to ensure the data sequence
    {
      ...
      _p = ecoPyth2.getPriceUnsafe("BTC");
      assertEq(_p.price, 92007.79015992e8);
      _p = ecoPyth2.getPriceUnsafe("ETH");
      assertEq(_p.price, 3017.54660183e8);
      _p = ecoPyth2.getPriceUnsafe("JPY");
      assertEq(_p.price, 1.49388473e8);
    }
  }

  function testCorrectness_UnsafeEcoPythCalldataBuilder3_shouldBuildTheSameResultAsSafeOne() external onlyFork {
    IEcoPythCalldataBuilder3.BuildData[] memory _data = _getCurrentBuildData();
    _overwritePriceByAssetId("EUR", _data, 1.05048e8, 15_000);
    _overwritePriceByAssetId("CHF", _data, 0.92e8, 15_000);
    _overwritePriceByAssetId("CAD", _data, 1.349e8, 15_000);
...
//   function testCorrectness_getGmTokenPrice() external onlyFork {
...
//   }
  ...
  function testCorrectness_build() external onlyFork {
    ...
    //   maxDiffBps: 15000
    // });
    IEcoPythCalldataBuilder3.BuildData[] memory buildData = new IEcoPythCalldataBuilder3.BuildData[](3);
    buildData[0] = IEcoPythCalldataBuilder3.BuildData({
      assetId: "ETH",
      priceE8: 3800.99 * 1e8,
      publishTime: uint160(block.timestamp),
  ...
  function testRevert_BadOrder() external onlyFork {
    IEcoPythCalldataBuilder3.BuildData[] memory buildData = new IEcoPythCalldataBuilder3.BuildData[](9);
    buildData[0] = IEcoPythCalldataBuilder3.BuildData({
      assetId: "ETH",
      priceE8: 1800.99 * 1e8,
      publishTime: uint160(block.timestamp),
  ...
  function testRevert_BadLength() external onlyFork {
